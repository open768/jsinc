...
class cQueueifVisible {
  ...
  onScrollingTimer() {
    ...
    bean.on(oQItem, "start", function () {
      oThis.onStart(oQItem)
    })
    bean.on(oQItem, "result", function (poHttp) {
      oThis.onResult(poHttp)
    })
    bean.on(oQItem, "error", function (poHttp) {
      oThis.onError(poHttp)
    })
    bean.on(oQItem, "Qpos", function () {
      oThis.onQPosition(oQItem)
    })
