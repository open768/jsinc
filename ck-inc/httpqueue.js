...
function cHttpQueue() {
  ...
  if (this instanceof cHttpQueue) {
    //not sure why i check whether its an instanceof
  ...
  this.onTimer = function (poItem) {
    ...
    bean.on(oHttp, "result", function (poHttp) {
      oThis.onResult(poHttp, poItem)
    })
    bean.on(oHttp, "error", function (poHttp) {
      oThis.onError(poHttp, poItem)
    })
  ...
  this.stop = function () {
    ...
    // eslint-disable-next-line no-unused-vars
    this.inProgressQ.forEach(function (oItem, psKey) {
      oItem.ohttp.stop()
    })
    this.inProgressQ = new Map()
  }
