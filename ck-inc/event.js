...
class cBaseEvent {
	...
	constructor(psBaseId, psAction, poData = null) {
		if (!psBaseId || !psAction) $.error('incorrect number of arguments')
		if (typeof bean === 'undefined') $.error('bean library is missing')

		if (this.constructor === cBaseEvent) $.error('cBaseEvent is abstract')
		// @ts-expect-error
	...
	static async fire_event(psGridName, psAction, poData = null) {
		if (this === cBaseEvent) throw new CAException('cBaseEvent is abstract')
		if (!psGridName) $.error('grid name is required')
		if (!psAction) $.error('grid name is required')
	...
	static async subscribe(psGridName, pfnCallback) {
		if (this === cBaseEvent) $.error('cBaseEvent is abstract')
		if (typeof pfnCallback !== 'function') $.error('callback must be a function')
		if (!psGridName) $.error('grid name is required')
